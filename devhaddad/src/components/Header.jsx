...
function Header() {
  ...
  useEffect(() => {
    ...
    fetch(ENDPOINT)
      .then(res => res.json())
      .then(response => {
        if (response?.data?.up_count !== undefined) setViews(response.data.up_count - response.data.down_count); // Subtrai os dislikes para mostrar apenas as visitas "positivas"
      })
  ...
  useEffect(() => {
    ...
    fetch(GET_LIKES)
      .then(res => res.json())
      .then(response => {
        if (response?.data?.up_count !== undefined) setLikes(response.data.up_count - response.data.down_count); // Subtrai os dislikes para mostrar apenas os likes "positivos"
      });
  }, [API_KEY]);
